...
## get the train data and lable the columns
...
## Rename all abbreviated variable names 
...
## Merge the activities database with the mean/std values database to get one dataset with descriptive activity names
 descriptionnames <- merge(activity_labels,meanstddata,by.x="activity_id",by.y="activity_id",all=TRUE)
 
## Melt the dataset with the descriptive activity names for better handling
 data_melt <- melt(descriptionnames,id=c("activity_id","activity_name","subject_id"))
